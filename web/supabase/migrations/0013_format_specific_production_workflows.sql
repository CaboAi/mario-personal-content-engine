-- Preserve meaningful progress when moving legacy all-purpose video states
-- into honest format-specific workflows.
-- The previous constraint permits only the older status matrix, so remove it
-- before migrating rows and replace it with the stricter matrix below.
alter table public.content_items
  drop constraint if exists content_items_format_status_check;

update public.content_items
set status = case
      when status = 'Scheduled' then 'Scheduled'::public.production_status
      when status in ('Recorded', 'Edited') then 'Final Copy'::public.production_status
      when status = 'Ready to Record' then 'Drafting'::public.production_status
      else 'Outline Ready'::public.production_status
    end,
    updated_at = now()
where format in ('Written Post', 'Long-form')
  and status <> 'Posted';

update public.content_items
set status = case
      when status in ('Ready to Record', 'Recorded', 'Edited', 'Scheduled') then status
      else 'Concept Ready'::public.production_status
    end,
    updated_at = now()
where format = 'POV / Realization'
  and status <> 'Posted';

update public.content_items
set status = 'Script Ready',
    updated_at = now()
where format in ('Yap Reel', 'Mini Story')
  and status not in ('Script Ready', 'Ready to Record', 'Recorded', 'Edited', 'Scheduled', 'Posted');

update public.content_items
set status = 'Copy Ready',
    updated_at = now()
where format = 'Carousel'
  and status not in ('Copy Ready', 'Designing in Canva', 'Design Ready', 'Posted');

alter table public.content_items
  add constraint content_items_format_status_check
  check (
    (
      format in ('Yap Reel', 'Mini Story')
      and status in ('Script Ready', 'Ready to Record', 'Recorded', 'Edited', 'Scheduled', 'Posted')
    )
    or
    (
      format = 'POV / Realization'
      and status in ('Concept Ready', 'Ready to Record', 'Recorded', 'Edited', 'Scheduled', 'Posted')
    )
    or
    (
      format = 'Carousel'
    ...
    or
    (
      format in ('Written Post', 'Long-form')
      and status in ('Outline Ready', 'Drafting', 'Final Copy', 'Scheduled', 'Posted')
    )
  );
